...
from MinimaxSearchBot import MinimaxSearchBot
from LocalSearchBot import LocalSearchBot
from time import time

#Настройка ткинтера
size_of_board = 500 
number_of_dots = 4 # Кількість точок
symbol_size = (size_of_board / 3 - size_of_board / 8) / 2
symbol_thickness = 500
dot_color = "black" 
player1_color = "#0000FF"
player1_color_light = "#6495ED"
player2_color = "#CD3333"
player2_color_light = "#FF4040"
Green_color = "#3D9140"
...
class Dots_and_Boxes:
    ...
    def play_again(self):
        ...
        # Введення від користувача у вигляді кліків
    ...
    def display_gameover(self):
        ...
        score_text = "Гравець 1 : " + str(player1_score) + "\n"
        score_text += "Гравець 2 : " + str(player2_score) + "\n"
        # score_text += 'НІчия                   : ' + str(self.tie_score)
        self.canvas.create_text(
            size_of_board / 2,
            3 * size_of_board / 4,
            font="cmr 30 bold",
        ...
        score_text = "Клік, щоб зіграти ще раз \n"
        self.canvas.create_text(
            size_of_board / 2,
            15 * size_of_board / 16,
            font="cmr 20 bold",
...
if __name__ == "__main__":
    root = Tk()
    root.title("Вибір Складності")
    root.geometry("250x200") 
 
    button = ttk.Button(text="Легко", command=Easy)
    button.pack(anchor=CENTER, expand=1)
    button1 = ttk.Button(text="Нормально", command=Normal)
    button1.pack(anchor=CENTER, expand=1)
    button2 = ttk.Button(text="Важко", command=Hard)
